...
class CLIPWithAdapter(nn.Module):
    ...
    def compute_prototypes(self, support_features: torch.Tensor, support_labels: torch.Tensor) -> torch.Tensor:
        ...
        for class_label in range(self.num_classes):
            ...
            # 图像和文本原型加权求和（确保同类别相加）
            multi_modal_prototype = 0.9 * image_prototype + 0.1 * text_prototype
...
def plot_tsne(features, labels, epoch, accuracy, save_path, dpi=1000):
    ...
    cbar.set_ticks(range(9))  # 设置刻度为 0-8
    cbar.set_ticklabels(range(1, 10))  # 设置标签为 1-9
...
for seed in seeds:
    ...
    # 初始化模型
    few_shot_classifier = CLIPWithAdapter(clip_model, num_classes=9, class_names=class_names).to(DEVICE)
    ...
    os.makedirs("1.0_Result", exist_ok=True)
    log_file = f"./1.0_Result/1.0_training_log.txt"

    # 训练过程
    with open(log_file, "w") as f:
        f.write(f"1.0_Training and Evaluation Log\n")
        f.write("======================================================\n")

    for epoch in range(n_epochs):
        print(f"Training foe Epoch {epoch}")
        ...
        evaluate_with_train_prototypes_and_tsne(few_shot_classifier, test_loader, prototypes, epoch=epoch + 1, device=DEVICE, log_file=log_file,save_path="./1.0_Result")
